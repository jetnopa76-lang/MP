          ...
          <div style={{ maxWidth: 900, margin: "0 auto", padding: "44px 24px 72px" }}>
            ...
            <div className="grid-products">
              {filtP.map(p => (<div key={p.id} onClick={() => { switchProduct(p); setStep(2); }} style={{ borderRadius: ds.cardRadius, overflow: "hidden", cursor: "pointer", background: ds.cardBg, border: ds.cardBorder !== false ? `1px solid ${ds.borderColor}` : "none", boxShadow: ds.cardShadow ? "0 4px 16px rgba(0,0,0,0.08)" : "none", transition: "all 0.3s" }}
                ...
                <div style={{ padding: `${ds.cardPadding}px` }}><div style={{ fontWeight: 600, fontSize: ds.cardNameSize, marginBottom: 3 }}>{p.name}</div><div style={{ fontSize: ds.cardDescSize, color: ds.mutedColor, marginBottom: 6, lineHeight: 1.4, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{p.desc}</div><div style={{ fontFamily: hf, fontSize: ds.cardPriceSize }}>From ${Math.min(...p.sizes.map(s => s.price))}</div></div>
              </div>))}
            </div>
          </div>
        </div>)}
            ...
            <div className="grid-customize">
              ...
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, padding: "8px 10px", borderRadius: 8, background: "#f0ede8" }}>
                  ...
                  <button onClick={() => { setPhotos([]); setStep(1); }} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "#bbb" }}>×</button>
