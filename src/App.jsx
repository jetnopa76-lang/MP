...
async function saveStore(key, val) {
  try {
    ...
    if (body.length > 2000000) {
      console.warn(`⚠ ${key}: payload is ${(body.length/1024/1024).toFixed(1)}MB`);
    }
    ...
    if (!res.ok) {
      ...
      if (err.includes("57014") || err.includes("timeout")) {
        console.warn("Upsert timed out for", key, "- trying delete+insert");
        ...
        if (!res2.ok) {
          ...
          alert(`Save failed for ${key} (${(body.length/1024/1024).toFixed(1)}MB). Data may be too large. Try using smaller images.`);
          throw new Error(err2);
        }
        console.log("✓ Delete+insert succeeded for", key);
        return;
      }
      throw new Error(err);
    }
    console.log("✓ Saved", key, `(${(body.length/1024).toFixed(0)}KB)`);
  } catch (e) {
...
function SceneSlot({ label, scene, onUpdate, onRemove }) {
  ...
  const upload = async e => {
    ...
    // Compress scene PNG - keep transparency but resize
    const raw = await r2d(f);
    ...
    onUpdate({ bgImage: compressed, frames: [{ ...DF, id: uid() }] });
  };
...
function AdminPanel({ products, setProducts, hero, setHero, pages, setPages, site, setSite, cartCfg, setCartCfg, uploadCfg, setUploadCfg, design, setDesign, orders, setOrders, save, onExit }) {
  ...
  const saveProd = async u => {
    let n;
    if (!u.id || !products.find(p => p.id === u.id)) { u.id = uid(); n = [...products, u]; } else { n = products.map(p => p.id === u.id ? u : p); }
    setProducts(n); setEditing(null);
    // Check payload size before saving
    const payload = JSON.stringify(n);
    if (payload.length > 3000000) {
      console.warn(`Products payload is ${(payload.length/1024/1024).toFixed(1)}MB — may be too large`);
    }
    await save("myphoto-products", n);
  };
