...
function SceneSlot({ label, scene, onUpdate, onRemove }) {
  const ref = useRef();
  const fr = scene?.frame || DF;
  const setFr = (k, v) => onUpdate({ ...scene, frame: { ...fr, [k]: Number(v) } });
  const upload = async e => { const f = e.target.files[0]; if (!f) return; const u = await r2d(f); onUpdate({ bgImage: u, frame: { ...DF } }); };
  return (
      ...
      {scene?.bgImage ? (
        <div style={{ borderRadius: 8, overflow: "hidden", border: "1px solid rgba(0,0,0,0.06)" }}>
          <div style={{ position: "relative", backgroundImage: "linear-gradient(45deg,#e0e0e0 25%,transparent 25%),linear-gradient(-45deg,#e0e0e0 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#e0e0e0 75%),linear-gradient(-45deg,transparent 75%,#e0e0e0 75%)", backgroundSize: "10px 10px", backgroundPosition: "0 0,0 5px,5px -5px,-5px 0" }}>
            ...
            <div style={{ position: "absolute", zIndex: 1, left: `${fr.x-fr.w/2}%`, top: `${fr.y-fr.h/2}%`, width: `${fr.w}%`, height: `${fr.h}%`, background: "linear-gradient(135deg,#7cb5e0,#b07ce0)", opacity: 0.5 }} />
            <div style={{ position: "absolute", zIndex: 3, left: `${fr.x-fr.w/2}%`, top: `${fr.y-fr.h/2}%`, width: `${fr.w}%`, height: `${fr.h}%`, border: "1.5px dashed rgba(255,255,255,0.85)", borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#fff", fontSize: 7, fontWeight: 700, textShadow: "0 1px 2px rgba(0,0,0,0.6)", fontFamily: F }}>PHOTO</span></div>
          ...
          <div style={{ padding: "6px 8px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 4 }}>
            {[{ l: "X", k: "x", mn: 5, mx: 95 }, { l: "Y", k: "y", mn: 5, mx: 95 }, { l: "W", k: "w", mn: 5, mx: 80 }, { l: "H", k: "h", mn: 5, mx: 80 }].map(c => (
              <div key={c.key || c.k}><div style={{ fontSize: 8, color: "#aaa", fontWeight: 600 }}>{c.l} {fr[c.k]}%</div><input type="range" min={c.mn} max={c.mx} value={fr[c.k]} onChange={e => setFr(c.k, e.target.value)} style={{ width: "100%", accentColor: "#1a1a1a" }} /></div>
            ))}
          </div>
        </div>
      ) : (
...
// ═══════════════════════════════════════
function ProductPreview({ imageUrl, product, sizeIdx, posX, posY, zoom, onPosChange, interactive, orientation }) {
  const zs = (zoom || 100) / 100;
  const sz = product.sizes[sizeIdx];
  // Pick scene based on photo orientation, with fallbacks
  ...
  const startDrag = (e) => {
    ...
    const startPosX = posX; const startPosY = posY;
    const rect = e.currentTarget.getBoundingClientRect();
    // Sensitivity: dragging across the full container width moves position 0→100
    ...
    const onMove = (ev) => {
      ...
      const dx = (cx - startX) * sensX;
      const dy = (cy - startY) * sensY;
      // Dragging right moves the photo right, which means the visible area shifts left (posX decreases)
      onPosChange({
  ...
  const onWheel = (e) => {
    ...
    onPosChange({ zoom: Math.max(100, Math.min(300, zoom + delta)) });
  };

  // The image is scaled up by zoom, and objectPosition pans it.
  // At zoom 100%: image fills via object-fit:cover, objectPosition controls crop anchor
  // At zoom 200%: image is 2x size, objectPosition pans within the larger image
  const photoStyle = {
    position: "absolute", inset: 0,
    width: `${zs * 100}%`, height: `${zs * 100}%`,
    maxWidth: "none",
    objectFit: "cover",
    objectPosition: `${posX}% ${posY}%`,
    // Center the scaled image, then offset by position
    left: `${-(zs - 1) * posX}%`,
    top: `${-(zs - 1) * posY}%`,
    display: "block",
  ...
  const cursorStyle = interactive ? (dragging ? "grabbing" : "grab") : "default";

  // Scene with real PNG
  if (sc?.bgImage) {
    const fr = sc.frame || DF;
    return (
      <div style={{ borderRadius: 16, overflow: "hidden", position: "relative", background: "#f0ede8" }}>
        ...
        <div
          onMouseDown={startDrag} onTouchStart={startDrag} onWheel={onWheel}
          style={{ position: "absolute", zIndex: 1, left: `${fr.x-fr.w/2}%`, top: `${fr.y-fr.h/2}%`, width: `${fr.w}%`, height: `${fr.h}%`, overflow: "hidden", cursor: cursorStyle, touchAction: "none" }}>
          <img src={imageUrl} alt="" draggable={false} style={photoStyle} />
        </div>
        ...
        {interactive && !dragging && (
          <div style={{ position: "absolute", top: 10, left: "50%", transform: "translateX(-50%)", zIndex: 3, background: "rgba(0,0,0,0.55)", color: "#fff", padding: "5px 14px", borderRadius: 20, fontSize: 11, fontFamily: F, fontWeight: 500, pointerEvents: "none", display: "flex", alignItems: "center", gap: 6 }}>
            <span>✋</span> Drag to position · Scroll to zoom
          </div>
        )}
        {interactive && (
  ...
  return (
    <div style={{ borderRadius: 16, overflow: "hidden", position: "relative", height: 300, background: "linear-gradient(135deg,#f0ebe4,#e0d8ce)" }}>
      ...
      <div
        onMouseDown={startDrag} onTouchStart={startDrag} onWheel={onWheel}
        style={{ position: "absolute", top: "8%", left: "50%", transform: "translateX(-50%)", width: "34%", height: "38%", borderRadius: 4, overflow: "hidden", boxShadow: "0 6px 24px rgba(0,0,0,0.2)", border: "3px solid rgba(255,255,255,0.7)", cursor: cursorStyle, touchAction: "none" }}>
        <img src={imageUrl} alt="" draggable={false} style={photoStyle} />
      </div>
...
export default function MyPhotoApp() {
  ...
  const detectOrientation = useCallback((dataUrl, cb) => {
    const img = document.createElement("img");
    img.onload = function() {
      const o = this.naturalWidth >= this.naturalHeight ? "landscape" : "portrait";
      cb(o);
    };
  ...
  const chgPhoto = useCallback(f => {
    ...
    r.onload = e => {
      const url = e.target.result;
      setImageUrl(url);
      detectOrientation(url, (o) => setPhotoOrientation(o));
    };
    r.readAsDataURL(f);
  }, [detectOrientation]);
  ...
  const addToCart = () => { setCart(p => [...p, { id: Date.now(), imageUrl, imageName, product: selProd, size: selProd.sizes[selSize].label, filter: "none", price, posX, posY, zoom }]); setStep(3); };
  const reset = () => { setImageUrl(null); setSelFilter("none"); setPosX(50); setPosY(50); setZoom(100); setSelSize(0); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#faf9f7", fontFamily: F, color: "#1a1a1a" }}>
      ...
      <div style={{ opacity: fadeIn ? 1 : 0, transform: fadeIn ? "translateY(0)" : "translateY(8px)", transition: "all 0.5s ease" }}>
        ...
        {step === 0 && (<div>
          ...
          <div style={{ maxWidth: 900, margin: "0 auto", padding: "44px 24px 72px" }}>
            ...
            <div className="grid-products">
              {filtP.map(p => (<div key={p.id} onClick={() => { setSelProd(p); setSelSize(0); setSelImg(0); setStep(1); }} style={{ borderRadius: 14, overflow: "hidden", cursor: "pointer", background: "#fff", border: "1px solid rgba(0,0,0,0.05)", transition: "all 0.3s" }}
        ...
        {/* CUSTOMIZE */}
        {step === 2 && imageUrl && (<div style={{ maxWidth: 980, margin: "0 auto", padding: "16px 24px 56px" }}>
          {!selProd && <div style={{ marginBottom: 20 }}><p style={secL}>Choose a product</p><div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 6 }}>{active.map(p => <button key={p.id} onClick={() => { setSelProd(p); setSelSize(0); setSelImg(0); }} style={{ flexShrink: 0, padding: 12, borderRadius: 10, border: "1px solid rgba(0,0,0,0.08)", background: "#fff", cursor: "pointer", textAlign: "center", minWidth: 110, fontFamily: F }}><div style={{ fontSize: 12, fontWeight: 600 }}>{p.name}</div><div style={{ fontSize: 11, color: "#aaa" }}>From ${Math.min(...p.sizes.map(s => s.price))}</div></button>)}</div></div>}
          {selProd && (() => {
            ...
            <div className="grid-customize">
              ...
              <div>
                {/* Preview */}
                <ProductPreview imageUrl={imageUrl} product={selProd} sizeIdx={selSize} posX={posX} posY={posY} zoom={zoom} interactive orientation={photoOrientation}
                  onPosChange={({ posX: px, posY: py, zoom: z }) => { if (px !== undefined) setPosX(px); if (py !== undefined) setPosY(py); if (z !== undefined) setZoom(z); }} />

                {/* Zoom control — compact bar right under preview */}
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, padding: "6px 10px", borderRadius: 8, background: "#f0ede8" }}>
                  <span style={{ fontSize: 13 }}>−</span>
                  <input type="range" min="100" max="300" value={zoom} onChange={e => setZoom(Number(e.target.value))} style={{ flex: 1, accentColor: ac }} />
                  <span style={{ fontSize: 13 }}>+</span>
                  <span style={{ fontSize: 11, fontWeight: 600, color: "#555", minWidth: 32 }}>{zoom}%</span>
                  {(posX !== 50 || posY !== 50 || zoom !== 100) && <button onClick={() => { setPosX(50); setPosY(50); setZoom(100); }} style={{ background: "none", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 6, padding: "2px 8px", cursor: "pointer", fontFamily: F, fontSize: 10, color: "#888" }}>Reset</button>}
                </div>
        ...
        {/* CUSTOM PAGE */}
        {step === 5 && curPage && <PageRenderer page={curPage} products={products} onProductClick={p => { setSelProd(p); setSelSize(0); setSelImg(0); setStep(1); }} />}
