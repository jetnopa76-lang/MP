...
// ─── Defaults ───
const DEF_HERO = {
  headline: "Your photos,", headlineLine2: "beautifully printed",
  subtitle: "Upload a photo, see it on our products, and order in minutes.",
  ctaText: "Upload Your Photo", bgImage: null, badge: "Transform your memories",
  // Styling
  bgColor: "#faf9f7", bgGradient: "linear-gradient(180deg,#faf9f7,#f0ede8)",
  overlayColor: "rgba(0,0,0,0.3)", overlayOpacity: 30,
  headlineFont: "serif", headlineSize: 56, headlineColor: "", headlineWeight: 400,
  line2Italic: true, line2Color: "",
  badgeColor: "", badgeSize: 11, badgeSpacing: 3,
  subtitleColor: "", subtitleSize: 15,
  ctaBg: "", ctaColor: "#ffffff", ctaRadius: 50, ctaPaddingX: 40, ctaPaddingY: 14, ctaSize: 14,
  paddingTop: 72, paddingBottom: 56,
  maxWidth: 520,
  showGuarantee: true,
};
const DEF_SITE = { logoText: "myphoto", logoImage: null, logoHeight: 32, accentColor: "#1a1a1a", freeShipMin: 109, guaranteeText: "100% Smile Guarantee", footerLinks: ["FAQ", "Reviews", "Privacy", "Terms"], adminPassword: "admin123", testMode: false, announcement: { enabled: false, text: "Free shipping on all orders over $109!", bgColor: "#1a1a1a", textColor: "#ffffff", fontSize: 12 }, navItems: [
...
const DEF_DESIGN = {
  ...
  bodyFont: "DM Sans",
  customFontUrl: "", // extra Google Font URL
  // Global colors
  bgColor: "#faf9f7",
  textColor: "#1a1a1a",
  mutedColor: "#999999",
  ...
  pdpPriceColor: "",
  pdpPriceWeight: 400,
  pdpPriceFont: "heading", // "heading" or "body"
  cardGap: 16,
  cardColumns: 3, // min columns on desktop
  cardShadow: false,
  cardBorder: true,
...
async function generatePrintPDF(item) {
  ...
  // Get scene frames for positioning
  const sizeObj = item.product?.sizes?.find(s => s.label === item.size);
  const photos = item.photos || [];
  const firstPhoto = photos[0] || (item.imageUrl ? { url: item.imageUrl, posX: item.posX ?? 50, posY: item.posY ?? 50, zoom: item.zoom ?? 100, orientation: "landscape" } : null);
  const isPortrait = firstPhoto?.orientation === "portrait";
  const scene = isPortrait
    ? (sizeObj?.scenePortrait || sizeObj?.sceneLandscape || sizeObj?.scene || null)
    : (sizeObj?.sceneLandscape || sizeObj?.scene || sizeObj?.scenePortrait || null);
  const frames = mkFrames(scene);

  // If we have frames, draw each photo in its frame position
  // If no frames, draw the first photo full-bleed
  const photosToDraw = frames.length > 0
    ? frames.map((fr, i) => ({ frame: fr, photo: photos[i] || null }))
    : [{ frame: { x: 50, y: 50, w: 100, h: 100 }, photo: firstPhoto }];

  for (const { frame, photo } of photosToDraw) {
    if (!photo?.url) continue;

    // Load image
    const img = document.createElement("img");
    img.crossOrigin = "anonymous";
    await new Promise((resolve, reject) => { img.onload = resolve; img.onerror = reject; img.src = photo.url; });

    // Calculate frame position in pixels
    const fxPx = ((frame.x - frame.w / 2) / 100) * pxW;
    const fyPx = ((frame.y - frame.h / 2) / 100) * pxH;
    const fwPx = (frame.w / 100) * pxW;
    const fhPx = (frame.h / 100) * pxH;

    // Apply zoom and position crop
    const zoomScale = (photo.zoom || 100) / 100;
    const px = (photo.posX ?? 50) / 100;
    const py = (photo.posY ?? 50) / 100;
    const imgRatio = img.width / img.height;
    const frameRatio = fwPx / fhPx;
    let cropW, cropH;
    ...
    // Clip to frame area and draw
    ctx.save();
    ctx.beginPath();
    ctx.rect(fxPx, fyPx, fwPx, fhPx);
    ctx.clip();
    ctx.drawImage(img, sx, sy, cropW, cropH, fxPx, fyPx, fwPx, fhPx);
    ctx.restore();
  }
  ...
  const photoCount = photosToDraw.filter(p => p.photo?.url).length;
  return { blob, dims, dpi: DPI, pxW, pxH, photoCount };
}

// Generate separate PDFs for multi-print products (e.g. set of 4 coasters)
async function generateMultiPrintPDFs(item) {
  const DPI = 300;
  const dims = parseSizeInches(item.size);
  const photos = item.photos || [];
  const results = [];
  ...
  for (let i = 0; i < photos.length; i++) {
    ...
    let cropW, cropH;
    if (imgRatio > canvasRatio) { cropH = img.height / zoomScale; cropW = cropH * canvasRatio; }
    else { cropW = img.width / zoomScale; cropH = cropW / canvasRatio; }
    const maxOffsetX = img.width - cropW;
    const maxOffsetY = img.height - cropH;
    const sx = Math.max(0, Math.min(maxOffsetX, px * maxOffsetX));
    const sy = Math.max(0, Math.min(maxOffsetY, py * maxOffsetY));
    ...
    const jpegData = canvas.toDataURL("image/jpeg", 0.95);
    const ptW = dims.w * 72; const ptH = dims.h * 72;
    const b64 = jpegData.split(",")[1];
    const binaryLen = atob(b64).length;
    const encoder = new TextEncoder();
    const pdfParts = []; const offs = [];
    let t1 = "%PDF-1.4\n";
    offs[1] = t1.length; t1 += "1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n";
    offs[2] = t1.length; t1 += "2 0 obj\n<< /Type /Pages /Kids [3 0 R] /Count 1 >>\nendobj\n";
    offs[3] = t1.length; t1 += `3 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${ptW.toFixed(2)} ${ptH.toFixed(2)}] /Contents 5 0 R /Resources << /XObject << /Img 4 0 R >> >> >>\nendobj\n`;
    offs[4] = t1.length; t1 += `4 0 obj\n<< /Type /XObject /Subtype /Image /Width ${pxW} /Height ${pxH} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${binaryLen} >>\nstream\n`;
    pdfParts.push(encoder.encode(t1));
    const bin = atob(b64); const ba = new Uint8Array(bin.length);
    for (let j = 0; j < bin.length; j++) ba[j] = bin.charCodeAt(j);
    pdfParts.push(ba);
    let t2 = "\nendstream\nendobj\n";
    const ss = `q ${ptW.toFixed(2)} 0 0 ${ptH.toFixed(2)} 0 0 cm /Img Do Q`;
    offs[5] = pdfParts.reduce((a, p) => a + p.length, 0) + t2.length;
    t2 += `5 0 obj\n<< /Length ${ss.length} >>\nstream\n${ss}\nendstream\nendobj\n`;
    const xo = pdfParts.reduce((a, p) => a + p.length, 0) + t2.length;
    t2 += `xref\n0 6\n0000000000 65535 f \n`;
    for (let k = 1; k <= 5; k++) t2 += `${String(offs[k] || 0).padStart(10, "0")} 00000 n \n`;
    t2 += `trailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n${xo}\n%%EOF`;
    pdfParts.push(encoder.encode(t2));

    results.push({ blob: new Blob(pdfParts, { type: "application/pdf" }), dims, dpi: DPI, pxW, pxH, photoIndex: i, photoName: photo.name });
  }
  return results;
}
...
const SUPA_SAVE_HEADERS = { ...SUPA_HEADERS, "Prefer": "resolution=merge-duplicates,return=minimal" };
...
async function saveStore(key, val) {
  try {
    ...
    // Check payload size — warn if very large
    if (body.length > 4000000) {
      console.warn(`Save payload for ${key} is ${(body.length/1024/1024).toFixed(1)}MB — may timeout`);
    }
    const res = await fetch(`${SUPABASE_URL}/rest/v1/site_config`, {
      method: "POST",
      headers: SUPA_SAVE_HEADERS,
      body,
    });
    if (!res.ok) {
      const err = await res.text();
      // If timeout, try delete then insert
      if (err.includes("57014") || err.includes("timeout")) {
        console.warn("Upsert timed out, trying delete+insert for", key);
        await fetch(`${SUPABASE_URL}/rest/v1/site_config?key=eq.${encodeURIComponent(key)}`, { method: "DELETE", headers: SUPA_HEADERS });
        const res2 = await fetch(`${SUPABASE_URL}/rest/v1/site_config`, { method: "POST", headers: SUPA_SAVE_HEADERS, body });
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(6px)" }}>
      <div style={{ background: "#fff", borderRadius: 20, width: "92%", maxWidth: 680, maxHeight: "92vh", overflow: "auto", padding: 28, position: "relative" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={lbl}>Product Attributes</label>
          <p style={{ fontSize: 11, color: "#bbb", marginBottom: 8 }}>Add custom fields like Color, Material, Weight, etc. Shown on the product page.</p>
          {(fm.attributes || []).map((attr, i) => (
            <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6, alignItems: "center" }}>
              <input value={attr.label} onChange={e => { const a = [...(fm.attributes||[])]; a[i] = { ...a[i], label: e.target.value }; set("attributes", a); }} style={{ ...inp, flex: 0.4 }} placeholder="Label (e.g. Color)" />
              <input value={attr.value} onChange={e => { const a = [...(fm.attributes||[])]; a[i] = { ...a[i], value: e.target.value }; set("attributes", a); }} style={{ ...inp, flex: 0.6 }} placeholder="Value (e.g. Crystal Clear)" />
              <button onClick={() => set("attributes", (fm.attributes||[]).filter((_, j) => j !== i))} style={{ background: "none", border: "none", color: "#ccc", cursor: "pointer", fontSize: 16 }}>×</button>
            </div>
          ))}
          <button onClick={() => set("attributes", [...(fm.attributes||[]), { label: "", value: "" }])} style={{ background: "none", border: "1px dashed rgba(0,0,0,0.12)", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 11, color: "#888", fontFamily: F }}>+ Add attribute</button>
        </div>
        {/* Print Mode */}
        <div style={{ marginBottom: 16 }}>
          <label style={lbl}>Print Mode (for multi-photo products)</label>
          <p style={{ fontSize: 11, color: "#bbb", marginBottom: 8 }}>Controls how PDFs are generated when the product has multiple photo slots.</p>
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => set("printMode", "single")} style={{ flex: 1, padding: "10px 12px", borderRadius: 10, border: (fm.printMode || "single") === "single" ? "2px solid #1a1a1a" : "1px solid rgba(0,0,0,0.1)", background: (fm.printMode || "single") === "single" ? "#1a1a1a" : "#fff", color: (fm.printMode || "single") === "single" ? "#fff" : "#555", cursor: "pointer", fontFamily: F, fontSize: 12, fontWeight: 500, textAlign: "center" }}>
              <div style={{ fontWeight: 700, marginBottom: 2 }}>Collage</div>
              <div style={{ fontSize: 10, opacity: 0.7 }}>All photos → 1 PDF</div>
              <div style={{ fontSize: 9, opacity: 0.5 }}>e.g. photo collage block</div>
            </button>
            <button onClick={() => set("printMode", "multi")} style={{ flex: 1, padding: "10px 12px", borderRadius: 10, border: fm.printMode === "multi" ? "2px solid #1a1a1a" : "1px solid rgba(0,0,0,0.1)", background: fm.printMode === "multi" ? "#1a1a1a" : "#fff", color: fm.printMode === "multi" ? "#fff" : "#555", cursor: "pointer", fontFamily: F, fontSize: 12, fontWeight: 500, textAlign: "center" }}>
              <div style={{ fontWeight: 700, marginBottom: 2 }}>Multi-Print</div>
              <div style={{ fontSize: 10, opacity: 0.7 }}>Each photo → its own PDF</div>
              <div style={{ fontSize: 9, opacity: 0.5 }}>e.g. set of 4 coasters</div>
            </button>
          </div>
        </div>
        {/* Sizes */}
...
//  ADMIN PANEL
// ═══════════════════════════════════════
function AdminPanel({ products, setProducts, hero, setHero, pages, setPages, site, setSite, cartCfg, setCartCfg, uploadCfg, setUploadCfg, design, setDesign, orders, setOrders, save, onExit }) {
  ...
  // Generate and download PDF(s) for a single order item
  const downloadItemPdf = async (item, orderId) => {
    if (!item.imageUrl && (!item.photos || !item.photos.some(p => p?.url))) {
      alert("Photo data not available — PDFs are generated at checkout and must be downloaded at that time.");
      return;
    }
    const isMulti = item.product?.printMode === "multi" && item.photos?.length > 1;
    if (isMulti) {
      const results = await generateMultiPrintPDFs(item);
      for (const r of results) {
        const url = URL.createObjectURL(r.blob);
        const a = document.createElement("a");
        a.href = url; a.download = `${item.product?.name || "print"}-${item.size.replace(/["""]/g, "")}-photo${r.photoIndex + 1}-${orderId}.pdf`;
  ...
  const tabBtns = [
    { id: "orders", label: `Orders (${orders.length})` }, { id: "products", label: "Products" }, { id: "design", label: "Design" }, { id: "hero", label: "Hero" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f4f2" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 24px 60px" }}>
        ...
        {tab === "orders" && (<>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                      ...
                      {order.isTest && <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 4, background: "#e8a020", color: "#fff", letterSpacing: 0.5 }}>TEST</span>}
                    </div>
                ...
                <div style={{ padding: "8px 18px" }}>
                  {(order.items || []).map((item, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", fontSize: 13, borderBottom: i < (order.items?.length || 0) - 1 ? "1px solid rgba(0,0,0,0.03)" : "none" }}>
                      ...
                      {item.photoCount > 1 && <span style={{ color: "#aaa" }}>· {item.photoCount} photos</span>}
                      {item.pdfCount > 1 && <span style={{ color: "#8844cc" }}>· {item.pdfCount} PDFs</span>}
          ...
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid rgba(0,0,0,0.06)" }}>
            <button onClick={async () => {
              ...
              setOrders([]);
              try {
                await saveStore("myphoto-orders", []);
              } catch(e) {
                // Direct delete if save fails
                try {
                  await fetch(`${SUPABASE_URL}/rest/v1/site_config?key=eq.myphoto-orders`, { method: "DELETE", headers: SUPA_HEADERS });
                  alert("Orders cleared successfully.");
                } catch(e2) { alert("Failed to clear: " + e2.message); }
              }
            }} style={{ background: "none", border: "1px solid rgba(200,50,50,0.2)", borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontFamily: F, fontSize: 12, color: "#d44" }}>Clear All Orders</button>
          </div>
        </>)}

        {/* ─── DESIGN TAB ─── */}
        {tab === "design" && (() => {
          const d = design || DEF_DESIGN;
          const setD = (k, v) => { const u = { ...d, [k]: v }; setDesign(u); save("myphoto-design", u); };
          const colorPick = (label, key, def) => (
            <div><div style={{ fontSize: 10, color: "#aaa", marginBottom: 2 }}>{label}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <input type="color" value={d[key] || def} onChange={e => setD(key, e.target.value)} style={{ width: 28, height: 24, border: "none", borderRadius: 4, cursor: "pointer", padding: 0 }} />
                <span style={{ fontSize: 9, color: "#bbb", fontFamily: "monospace" }}>{d[key] || def}</span>
                {d[key] && d[key] !== def && <button onClick={() => setD(key, def)} style={{ background: "none", border: "none", color: "#ccc", cursor: "pointer", fontSize: 9 }}>↺</button>}
              </div>
            </div>
          );
          const slider = (label, key, min, max, unit) => (
            <div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#aaa" }}><span>{label}</span><span>{d[key]}{unit || "px"}</span></div>
              <input type="range" min={min} max={max} value={d[key]} onChange={e => setD(key, Number(e.target.value))} style={{ width: "100%", accentColor: "#1a1a1a" }} />
            </div>
          );
          const GFONTS = ["DM Sans", "Instrument Serif", "Inter", "Poppins", "Montserrat", "Playfair Display", "Lora", "Raleway", "Open Sans", "Roboto", "Oswald", "Merriweather", "Nunito", "Work Sans", "Libre Baskerville", "Cormorant Garamond", "Crimson Text", "Source Sans 3", "Lato", "PT Serif"];
          return (<>
            <h2 style={{ fontFamily: S, fontSize: 26, marginBottom: 16 }}>Design System</h2>

            {/* Typography */}
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Typography</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                <div><label style={lbl}>Heading Font</label>
                  <select value={d.headingFont} onChange={e => setD("headingFont", e.target.value)} style={inp}>
                    {GFONTS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                </div>
                <div><label style={lbl}>Body Font</label>
                  <select value={d.bodyFont} onChange={e => setD("bodyFont", e.target.value)} style={inp}>
                    {GFONTS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                </div>
              </div>
              <div><label style={lbl}>Custom Google Font URL (optional)</label>
                <input value={d.customFontUrl || ""} onChange={e => setD("customFontUrl", e.target.value)} style={inp} placeholder="https://fonts.googleapis.com/css2?family=..." />
                <p style={{ fontSize: 10, color: "#bbb", marginTop: 4 }}>Paste a Google Fonts embed URL to load additional fonts. Then select them above.</p>
              </div>
              {/* Preview */}
              <div style={{ marginTop: 12, padding: 14, borderRadius: 10, background: "#faf9f7", border: "1px solid rgba(0,0,0,0.04)" }}>
                <div style={{ fontFamily: `'${d.headingFont}', serif`, fontSize: 22, marginBottom: 4 }}>Heading Font Preview</div>
                <div style={{ fontFamily: `'${d.bodyFont}', sans-serif`, fontSize: 14, color: "#666" }}>Body font preview — The quick brown fox jumps over the lazy dog. 0123456789</div>
              </div>
            </div>

            {/* Colors */}
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Colors</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
                {colorPick("Background", "bgColor", "#faf9f7")}
                {colorPick("Text", "textColor", "#1a1a1a")}
                {colorPick("Muted Text", "mutedColor", "#999999")}
                {colorPick("Card Bg", "cardBg", "#ffffff")}
              </div>
            </div>

            {/* Navigation */}
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Navigation</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                {colorPick("Nav Background", "navBg", "rgba(250,249,247,0.92)")}
                {colorPick("Nav Text", "navTextColor", "#555555")}
                <div>{slider("Font Size", "navFontSize", 10, 18)}</div>
              </div>
              {slider("Nav Padding", "navPadding", 6, 24)}
            </div>

            {/* Product Cards */}
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Product Cards</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
                {slider("Card Radius", "cardRadius", 0, 30)}
                {slider("Image Height", "cardImgHeight", 100, 300)}
                {slider("Card Padding", "cardPadding", 8, 30)}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
                {slider("Name Size", "cardNameSize", 10, 22)}
                {slider("Desc Size", "cardDescSize", 9, 16)}
                {slider("Price Size", "cardPriceSize", 12, 26)}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                {colorPick("Price Color", "cardPriceColor", "#1a1a1a")}
                <div><label style={lbl}>Price Weight</label>
                  <select value={d.cardPriceWeight || 400} onChange={e => setD("cardPriceWeight", Number(e.target.value))} style={inp}>
                    <option value={300}>Light</option><option value={400}>Regular</option><option value={500}>Medium</option><option value={600}>Semi-bold</option><option value={700}>Bold</option>
                  </select>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                {slider("Grid Gap", "cardGap", 4, 32)}
                {slider("Min Columns", "cardColumns", 2, 5, "")}
              </div>
              <div style={{ display: "flex", gap: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <button onClick={() => setD("cardShadow", !d.cardShadow)} style={{ width: 36, height: 20, borderRadius: 10, border: "none", cursor: "pointer", background: d.cardShadow ? "#1a1a1a" : "#ddd", position: "relative" }}><div style={{ width: 14, height: 14, borderRadius: "50%", background: "#fff", position: "absolute", top: 3, left: d.cardShadow ? 19 : 3, transition: "left 0.2s" }} /></button>
                  <span style={{ fontSize: 11, color: "#666" }}>Shadow</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <button onClick={() => setD("cardBorder", !d.cardBorder)} style={{ width: 36, height: 20, borderRadius: 10, border: "none", cursor: "pointer", background: d.cardBorder !== false ? "#1a1a1a" : "#ddd", position: "relative" }}><div style={{ width: 14, height: 14, borderRadius: "50%", background: "#fff", position: "absolute", top: 3, left: d.cardBorder !== false ? 19 : 3, transition: "left 0.2s" }} /></button>
                  <span style={{ fontSize: 11, color: "#666" }}>Border</span>
                </div>
              </div>
            </div>
            ...
            {/* Product Detail */}
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Product Detail & Editor</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                {slider("Title Size", "pdpNameSize", 18, 40)}
                {slider("Description Size", "pdpDescSize", 11, 20)}
                {slider("Preview Radius", "pdpPreviewRadius", 0, 30)}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginTop: 12 }}>
                {slider("Price Size", "pdpPriceSize", 14, 40)}
                {colorPick("Price Color", "pdpPriceColor", "#1a1a1a")}
                <div><label style={lbl}>Price Weight</label>
                  <select value={d.pdpPriceWeight || 400} onChange={e => setD("pdpPriceWeight", Number(e.target.value))} style={inp}>
                    <option value={300}>Light</option><option value={400}>Regular</option><option value={500}>Medium</option><option value={600}>Semi-bold</option><option value={700}>Bold</option>
                  </select>
                </div>
              </div>
              <div style={{ marginTop: 8 }}><label style={lbl}>Price Font</label>
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => setD("pdpPriceFont", "heading")} style={{ padding: "6px 14px", borderRadius: 7, border: "none", cursor: "pointer", fontFamily: F, fontSize: 12, fontWeight: 500, background: (d.pdpPriceFont || "heading") === "heading" ? "#1a1a1a" : "#e8e6e2", color: (d.pdpPriceFont || "heading") === "heading" ? "#fff" : "#666" }}>Heading</button>
                  <button onClick={() => setD("pdpPriceFont", "body")} style={{ padding: "6px 14px", borderRadius: 7, border: "none", cursor: "pointer", fontFamily: F, fontSize: 12, fontWeight: 500, background: d.pdpPriceFont === "body" ? "#1a1a1a" : "#e8e6e2", color: d.pdpPriceFont === "body" ? "#fff" : "#666" }}>Body</button>
                </div>
              </div>
            </div>
            ...
            {/* Buttons */}
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Buttons</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                {slider("Button Radius", "btnRadius", 0, 30)}
                {slider("Button Font Size", "btnFontSize", 10, 20)}
                {slider("Button Padding", "btnPadding", 8, 24)}
              </div>
              {slider("CTA Pill Radius", "ctaBtnRadius", 0, 50)}
            </div>

            {/* Layout */}
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Layout & Spacing</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
                {slider("Section Padding", "sectionPadding", 16, 80)}
                {slider("Page Pad X", "pagePaddingX", 12, 48)}
                {slider("Cart Width", "cartMaxWidth", 350, 700)}
                {slider("Checkout Width", "checkoutMaxWidth", 500, 900)}
              </div>
            </div>

            {/* Footer */}
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Footer</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                {colorPick("Footer Bg", "footerBg", "transparent")}
                {colorPick("Footer Text", "footerTextColor", "#bbbbbb")}
                {slider("Footer Font Size", "footerFontSize", 8, 16)}
              </div>
            </div>

            {/* Reset */}
            <button onClick={() => { setDesign(DEF_DESIGN); save("myphoto-design", DEF_DESIGN); }} style={{ background: "none", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontFamily: F, fontSize: 12, color: "#d44" }}>Reset all design to defaults</button>
          </>);
        })()}
        ...
        {tab === "hero" && (() => {
          const h = hero;
          const setH = (k, v) => { const u = { ...h, [k]: v }; setHero(u); save("myphoto-hero", u); };
          const colorInput = (label, key, def) => (
            <div><div style={{ fontSize: 10, color: "#aaa", marginBottom: 2 }}>{label}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <input type="color" value={h[key] || def} onChange={e => setH(key, e.target.value)} style={{ width: 28, height: 24, border: "none", borderRadius: 4, cursor: "pointer", padding: 0 }} />
                {h[key] && <button onClick={() => setH(key, "")} style={{ background: "none", border: "none", color: "#ccc", cursor: "pointer", fontSize: 10 }}>×</button>}
              </div>
            </div>
          );
          const numInput = (label, key, min, max, step) => (
            <div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#aaa" }}><span>{label}</span><span>{h[key]}</span></div>
              <input type="range" min={min} max={max} step={step || 1} value={h[key]} onChange={e => setH(key, Number(e.target.value))} style={{ width: "100%", accentColor: "#1a1a1a" }} />
            </div>
          );
          return (<>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, alignItems: "start" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ background: "#fff", borderRadius: 14, padding: 18, border: "1px solid rgba(0,0,0,0.06)" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>Content</div>
                  <div style={{ marginBottom: 10 }}><label style={lbl}>Badge</label><input value={h.badge} onChange={e => setH("badge", e.target.value)} style={inp} /></div>
                  <div style={{ marginBottom: 10 }}><label style={lbl}>Headline Line 1</label><input value={h.headline} onChange={e => setH("headline", e.target.value)} style={inp} /></div>
                  <div style={{ marginBottom: 10 }}><label style={lbl}>Headline Line 2</label><input value={h.headlineLine2} onChange={e => setH("headlineLine2", e.target.value)} style={inp} /></div>
                  <div style={{ marginBottom: 10 }}><label style={lbl}>Subtitle</label><textarea value={h.subtitle} onChange={e => setH("subtitle", e.target.value)} rows={2} style={{ ...inp, resize: "vertical" }} /></div>
                  <div><label style={lbl}>CTA Text</label><input value={h.ctaText} onChange={e => setH("ctaText", e.target.value)} style={inp} /></div>
                </div>
                {/* Typography */}
                <div style={{ background: "#fff", borderRadius: 14, padding: 18, border: "1px solid rgba(0,0,0,0.06)" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>Typography</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
                    <div><label style={lbl}>Headline Font</label>
                      <select value={h.headlineFont || "serif"} onChange={e => setH("headlineFont", e.target.value)} style={inp}>
                        <option value="serif">Serif (Instrument Serif)</option>
                        <option value="sans">Sans-serif (DM Sans)</option>
                      </select>
                    </div>
                    <div>{numInput("Headline Size (px)", "headlineSize", 24, 80)}</div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                    <div><label style={lbl}>Weight</label>
                      <select value={h.headlineWeight || 400} onChange={e => setH("headlineWeight", Number(e.target.value))} style={inp}>
                        <option value={300}>Light</option><option value={400}>Regular</option><option value={500}>Medium</option><option value={600}>Semi-bold</option><option value={700}>Bold</option>
                      </select>
                    </div>
                    <div style={{ display: "flex", alignItems: "end", gap: 6, paddingBottom: 2 }}>
                      <button onClick={() => setH("line2Italic", !h.line2Italic)} style={{ padding: "6px 12px", borderRadius: 6, border: h.line2Italic ? "2px solid #1a1a1a" : "1px solid rgba(0,0,0,0.1)", background: h.line2Italic ? "#1a1a1a" : "#fff", color: h.line2Italic ? "#fff" : "#888", cursor: "pointer", fontFamily: S, fontSize: 14, fontStyle: "italic" }}>I</button>
                    </div>
                    {numInput("Subtitle Size", "subtitleSize", 10, 24)}
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    {numInput("Badge Size", "badgeSize", 8, 18)}
                    {numInput("Badge Spacing", "badgeSpacing", 0, 8)}
                  </div>
                </div>
                {/* Colors */}
                <div style={{ background: "#fff", borderRadius: 14, padding: 18, border: "1px solid rgba(0,0,0,0.06)" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>Colors</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                    {colorInput("Headline", "headlineColor", "#1a1a1a")}
                    {colorInput("Line 2", "line2Color", "#1a1a1a")}
                    {colorInput("Badge", "badgeColor", "#aaaaaa")}
                    {colorInput("Subtitle", "subtitleColor", "#777777")}
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                    {colorInput("CTA Bg", "ctaBg", "#1a1a1a")}
                    {colorInput("CTA Text", "ctaColor", "#ffffff")}
                    {colorInput("Page Bg", "bgColor", "#faf9f7")}
                  </div>
                </div>
                {/* Spacing & Layout */}
                <div style={{ background: "#fff", borderRadius: 14, padding: 18, border: "1px solid rgba(0,0,0,0.06)" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>Spacing & Button</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
                    {numInput("Top Padding", "paddingTop", 20, 150)}
                    {numInput("Bottom Padding", "paddingBottom", 20, 150)}
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                    {numInput("CTA Radius", "ctaRadius", 0, 50)}
                    {numInput("CTA Pad X", "ctaPaddingX", 10, 60)}
                    {numInput("CTA Size", "ctaSize", 10, 22)}
                  </div>
                  {numInput("Max Width", "maxWidth", 300, 800)}
                  {h.bgImage && numInput("Overlay Opacity", "overlayOpacity", 0, 80)}
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
                    <button onClick={() => setH("showGuarantee", !h.showGuarantee)} style={{ width: 36, height: 20, borderRadius: 10, border: "none", cursor: "pointer", background: h.showGuarantee !== false ? "#1a1a1a" : "#ddd", position: "relative" }}><div style={{ width: 14, height: 14, borderRadius: "50%", background: "#fff", position: "absolute", top: 3, left: h.showGuarantee !== false ? 19 : 3, transition: "left 0.2s" }} /></button>
                    <span style={{ fontSize: 11, color: "#666" }}>Show guarantee text</span>
                  </div>
                </div>
                {/* Background Image */}
                <div style={{ background: "#fff", borderRadius: 14, padding: 18, border: "1px solid rgba(0,0,0,0.06)" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>Background Image</div>
                  {h.bgImage ? (
                    <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", marginBottom: 8 }}>
                      <img src={h.bgImage} alt="" style={{ width: "100%", display: "block", maxHeight: 140, objectFit: "cover" }} />
                      <button onClick={() => setH("bgImage", null)} style={{ position: "absolute", top: 4, right: 4, width: 22, height: 22, borderRadius: "50%", background: "rgba(0,0,0,0.6)", color: "#fff", border: "none", cursor: "pointer", fontSize: 11 }}>×</button>
                    </div>
                  ) : null}
                  <button onClick={() => heroRef.current?.click()} style={{ padding: "8px 14px", borderRadius: 8, border: "2px dashed rgba(0,0,0,0.08)", background: "transparent", cursor: "pointer", fontSize: 11, color: "#888", fontFamily: F }}>{h.bgImage ? "Change" : "Upload image"}</button>
                  <input ref={heroRef} type="file" accept="image/*" style={{ display: "none" }} onChange={async e => { const f = e.target.files[0]; if (!f) return; const u = await readAndCompress(f, 1600, 900, 0.75); setH("bgImage", u); }} />
                </div>
              </div>
              {/* Live Preview */}
              <div style={{ position: "sticky", top: 70 }}>
                <label style={lbl}>Live Preview</label>
                <div style={{ borderRadius: 14, overflow: "hidden", border: "1px solid rgba(0,0,0,0.06)", background: h.bgImage ? `url(${h.bgImage}) center/cover` : (h.bgColor || "#faf9f7") }}>
                  {h.bgImage && <div style={{ position: "absolute", inset: 0, background: `rgba(0,0,0,${(h.overlayOpacity || 30) / 100})`, borderRadius: 14 }} />}
                  <div style={{ position: "relative", zIndex: 1, padding: `${(h.paddingTop || 72) * 0.4}px 16px ${(h.paddingBottom || 56) * 0.4}px`, textAlign: "center" }}>
                    <p style={{ fontSize: (h.badgeSize || 11) * 0.7, letterSpacing: h.badgeSpacing || 3, textTransform: "uppercase", color: h.badgeColor || (h.bgImage ? "rgba(255,255,255,0.7)" : "#aaa"), marginBottom: 8, fontWeight: 500 }}>{h.badge}</p>
                    <h1 style={{ fontFamily: (h.headlineFont === "sans") ? F : S, fontSize: (h.headlineSize || 56) * 0.45, fontWeight: h.headlineWeight || 400, lineHeight: 1.1, margin: "0 auto 8px", color: h.headlineColor || (h.bgImage ? "#fff" : "#1a1a1a") }}>
                      {h.headline}<br /><span style={{ fontStyle: h.line2Italic !== false ? "italic" : "normal", color: h.line2Color || undefined }}>{h.headlineLine2}</span>
                    </h1>
                    <p style={{ fontSize: (h.subtitleSize || 15) * 0.7, color: h.subtitleColor || (h.bgImage ? "rgba(255,255,255,0.85)" : "#777"), margin: "0 auto 14px", lineHeight: 1.5 }}>{h.subtitle}</p>
                    <button style={{ background: h.ctaBg || "#1a1a1a", color: h.ctaColor || "#fff", border: "none", padding: `${(h.ctaPaddingY || 14) * 0.6}px ${(h.ctaPaddingX || 40) * 0.6}px`, borderRadius: h.ctaRadius || 50, fontSize: (h.ctaSize || 14) * 0.7, fontFamily: F, fontWeight: 600 }}>{h.ctaText}</button>
                    {h.showGuarantee !== false && <p style={{ fontSize: 8, color: h.bgImage ? "rgba(255,255,255,0.5)" : "#bbb", marginTop: 6 }}>Free shipping · Guarantee</p>}
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* ─── SITE SETTINGS TAB ─── */}
        {tab === "site" && (() => {
          const logoRef2 = document.createElement("input"); // inline ref trick
          const ann = site.announcement || { enabled: false, text: "", bgColor: "#1a1a1a", textColor: "#ffffff", fontSize: 12 };
          const setAnn = (k, v) => { const s = { ...site, announcement: { ...ann, [k]: v } }; setSite(s); save("myphoto-site", s); };
          return (<>
          <h2 style={{ fontFamily: S, fontSize: 26, marginBottom: 16 }}>Site Settings</h2>

          {/* Logo */}
          <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Logo</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div>
                <label style={lbl}>Logo Text (fallback)</label>
                <input value={site.logoText} onChange={e => { const s = { ...site, logoText: e.target.value }; setSite(s); save("myphoto-site", s); }} style={inp} />
              </div>
              <div>
                <label style={lbl}>Logo Height (px)</label>
                <input type="range" min={20} max={60} value={site.logoHeight || 32} onChange={e => { const s = { ...site, logoHeight: Number(e.target.value) }; setSite(s); save("myphoto-site", s); }} style={{ width: "100%", accentColor: "#1a1a1a" }} />
                <span style={{ fontSize: 10, color: "#aaa" }}>{site.logoHeight || 32}px</span>
              </div>
            </div>
            <div style={{ marginTop: 12 }}>
              ...
              {site.logoImage ? (
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                  <div style={{ padding: "8px 16px", borderRadius: 8, background: "#f5f4f2", border: "1px solid rgba(0,0,0,0.06)" }}>
                    <img src={site.logoImage} alt="Logo" style={{ height: site.logoHeight || 32, display: "block" }} />
                  </div>
                  <button onClick={() => { const s = { ...site, logoImage: null }; setSite(s); save("myphoto-site", s); }} style={{ background: "none", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11, color: "#888", fontFamily: F }}>Remove</button>
                </div>
              ) : null}
              <button onClick={() => {
                const i = document.createElement("input"); i.type = "file"; i.accept = "image/*";
                i.onchange = async e => { const f = e.target.files[0]; if (!f) return; const u = await readAndCompress(f, 400, 100, 0.9); const s = { ...site, logoImage: u }; setSite(s); save("myphoto-site", s); };
                i.click();
              }} style={{ padding: "8px 14px", borderRadius: 8, border: "2px dashed rgba(0,0,0,0.08)", background: "transparent", cursor: "pointer", fontSize: 11, color: "#888", fontFamily: F }}>{site.logoImage ? "Change logo" : "Upload logo image"}</button>
              <p style={{ fontSize: 10, color: "#bbb", marginTop: 4 }}>PNG with transparency recommended. Falls back to logo text if not set.</p>
            </div>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: 0.5 }}>Announcement Bar</div>
              <button onClick={() => setAnn("enabled", !ann.enabled)} style={{ width: 44, height: 24, borderRadius: 12, border: "none", cursor: "pointer", background: ann.enabled ? "#1a1a1a" : "#ddd", position: "relative" }}><div style={{ width: 18, height: 18, borderRadius: "50%", background: "#fff", position: "absolute", top: 3, left: ann.enabled ? 23 : 3, transition: "left 0.2s" }} /></button>
            </div>
            {ann.enabled && (<>
              <div style={{ marginBottom: 10 }}><label style={lbl}>Message</label><input value={ann.text} onChange={e => setAnn("text", e.target.value)} style={inp} placeholder="Free shipping on all orders!" /></div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                <div><div style={{ fontSize: 10, color: "#aaa", marginBottom: 2 }}>Background</div>
                  <input type="color" value={ann.bgColor || "#1a1a1a"} onChange={e => setAnn("bgColor", e.target.value)} style={{ width: 28, height: 24, border: "none", borderRadius: 4, cursor: "pointer", padding: 0 }} />
                </div>
                <div><div style={{ fontSize: 10, color: "#aaa", marginBottom: 2 }}>Text Color</div>
                  <input type="color" value={ann.textColor || "#ffffff"} onChange={e => setAnn("textColor", e.target.value)} style={{ width: 28, height: 24, border: "none", borderRadius: 4, cursor: "pointer", padding: 0 }} />
                </div>
                <div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#aaa" }}><span>Font Size</span><span>{ann.fontSize || 12}px</span></div>
                  <input type="range" min={9} max={16} value={ann.fontSize || 12} onChange={e => setAnn("fontSize", Number(e.target.value))} style={{ width: "100%", accentColor: "#1a1a1a" }} />
                </div>
              </div>
              {/* Preview */}
              <div style={{ borderRadius: 8, overflow: "hidden" }}>
                <div style={{ background: ann.bgColor || "#1a1a1a", color: ann.textColor || "#fff", textAlign: "center", padding: "7px 12px", fontSize: ann.fontSize || 12, fontWeight: 500, fontFamily: F }}>{ann.text || "Your announcement here"}</div>
              </div>
            </>)}
          </div>
          ...
          {/* General */}
          <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>General</div>
            <div style={{ marginBottom: 12 }}><label style={lbl}>Accent Color</label>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <input type="color" value={site.accentColor} onChange={e => { const s = { ...site, accentColor: e.target.value }; setSite(s); save("myphoto-site", s); }} style={{ width: 32, height: 28, border: "none", borderRadius: 6, cursor: "pointer", padding: 0 }} />
                <span style={{ fontSize: 11, color: "#888", fontFamily: "monospace" }}>{site.accentColor}</span>
              </div>
            </div>
            <div style={{ marginBottom: 12 }}><label style={lbl}>Guarantee Text</label><input value={site.guaranteeText} onChange={e => { const s = { ...site, guaranteeText: e.target.value }; setSite(s); save("myphoto-site", s); }} style={inp} /></div>
            <div style={{ marginBottom: 12 }}><label style={lbl}>Footer Links (comma-separated)</label><input value={site.footerLinks.join(", ")} onChange={e => { const s = { ...site, footerLinks: e.target.value.split(",").map(l => l.trim()).filter(Boolean) }; setSite(s); save("myphoto-site", s); }} style={inp} /></div>
          </div>

          {/* Admin */}
          <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#555", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Admin & Testing</div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              <button onClick={() => { const s = { ...site, testMode: !site.testMode }; setSite(s); save("myphoto-site", s); }} style={{ width: 44, height: 24, borderRadius: 12, border: "none", cursor: "pointer", background: site.testMode ? "#e8a020" : "#ddd", position: "relative" }}><div style={{ width: 18, height: 18, borderRadius: "50%", background: "#fff", position: "absolute", top: 3, left: site.testMode ? 23 : 3, transition: "left 0.2s" }} /></button>
              <div><span style={{ fontSize: 13, fontWeight: 600, color: site.testMode ? "#e8a020" : "#555" }}>Test Mode {site.testMode ? "ON" : "OFF"}</span>
                <div style={{ fontSize: 10, color: "#999" }}>Checkout shows a test order button that skips payment.</div></div>
            </div>
            <div><label style={lbl}>Admin Password</label>
              <p style={{ fontSize: 10, color: "#bbb", marginBottom: 6 }}>Access admin at <strong>yoursite.com/#admin</strong> or <strong>Ctrl+Shift+A</strong></p>
              <input type="text" value={site.adminPassword || ""} onChange={e => { const s = { ...site, adminPassword: e.target.value }; setSite(s); save("myphoto-site", s); }} style={inp} />
            </div>
          </div>
        </>);
        })()}
...
export default function MyPhotoApp() {
  ...
  const [design, setDesign] = useState(DEF_DESIGN);
  const [orders, setOrders] = useState([]);
  const [loaded, setLoaded] = useState(false);

  const [step, setStep] = useState(0); // 0=home,1=upload,2=customize,3=cart,4=done,5=page
  const [photosByProduct, setPhotosByProduct] = useState({}); // { [productId]: [{ url, name, posX, posY, zoom, orientation }] }
  const [activeFrame, setActiveFrame] = useState(0);
  const [uploadedPool, setUploadedPool] = useState([]); // global pool: [{ url, name, orientation }]
  ...
  useEffect(() => {
    (async () => {
      ...
      setDesign(await loadStore("myphoto-design", DEF_DESIGN));
      const rawOrders = await loadStore("myphoto-orders", []);
      // Clean old orders that may contain large imageUrl data
      const cleanOrders = rawOrders.map(o => ({
        ...o,
        items: (o.items || []).map(it => {
          const { imageUrl, photos, imageThumb, ...rest } = it;
          return rest;
        })
      }));
      setOrders(cleanOrders);
      // Save cleaned orders back if they were dirty
      if (JSON.stringify(cleanOrders).length < JSON.stringify(rawOrders).length) {
        try {
          await saveStore("myphoto-orders", cleanOrders);
        } catch(e) {
          console.warn("Clean save failed, deleting bloated orders:", e);
          try {
            await fetch(`${SUPABASE_URL}/rest/v1/site_config?key=eq.myphoto-orders`, { method: "DELETE", headers: SUPA_HEADERS });
            await saveStore("myphoto-orders", cleanOrders);
          } catch(e2) { console.warn("Delete also failed:", e2); }
        }
      }
      setLoaded(true);
    })();
  ...
  const addToPool = useCallback((url, name, orientation) => {
    setUploadedPool(prev => {
      if (prev.some(p => p.url === url)) return prev;
      return [...prev, { url, name, orientation }];
    });
  }, []);

  const removeFromPool = useCallback((idx) => {
    setUploadedPool(prev => prev.filter((_, i) => i !== idx));
  }, []);
  ...
  const handleFile = useCallback(f => {
    ...
    r.onload = e => {
      ...
      detectOrientation(url, (o) => {
        ...
        addToPool(url, f.name, o);
        setPhotosByProduct(prev => {
          const key = selProd ? selProd.id : "_temp";
    ...
    r.readAsDataURL(f);
  }, [detectOrientation, activeFrame, selProd]);
    ...
    files.forEach((f, i) => {
      ...
      r.onload = e => {
        ...
        detectOrientation(url, (o) => {
          ...
          if (fi === 0) { setPhotoOrientation(o); }
          loaded++;
          if (loaded === files.length) setStep(2);
        });
      };
      r.readAsDataURL(f);
    });
  }, [detectOrientation, selProd]);

  const handleDrop = useCallback(e => { e.preventDefault(); setDragO(false); if (e.dataTransfer.files.length > 1) handleFiles(e.dataTransfer.files); else handleFile(e.dataTransfer.files[0]); }, [handleFile, handleFiles]);
  ...
  const photos = selProd ? (photosByProduct[selProd.id] || []) : (photosByProduct["_temp"] || []);
  ...
  // imageUrl is derived — always in sync with first photo of current product
  const imageUrl = photos[0]?.url || null;
  const imageName = photos[0]?.name || "";
  ...
  const ds = design || DEF_DESIGN;
  const hf = `'${ds.headingFont}', serif`;
  const bf = `'${ds.bodyFont}', sans-serif`;
  ...
  const reset = () => { setSelFilter("none"); setPosX(50); setPosY(50); setZoom(100); setSelSize(0); setActiveFrame(0); setPhotosByProduct({}); };
  const switchProduct = (p) => {
    // Transfer temp photos to the new product if it has none
    const tempPhotos = photosByProduct["_temp"] || [];
    const existingPhotos = photosByProduct[p.id] || [];
    if (tempPhotos.length > 0 && existingPhotos.length === 0) {
      setPhotosByProduct(prev => ({ ...prev, [p.id]: tempPhotos, "_temp": [] }));
    }
    setSelProd(p); setSelSize(0); setSelImg(0); setActiveFrame(0);
    const savedPhotos = photosByProduct[p.id] || tempPhotos;
    if (savedPhotos.length > 0 && savedPhotos[0]?.url) {
      setPhotoOrientation(savedPhotos[0].orientation || "landscape");
      setPosX(savedPhotos[0].posX ?? 50);
      setPosY(savedPhotos[0].posY ?? 50);
      setZoom(savedPhotos[0].zoom ?? 100);
    }
  };
  const save = async (k, v) => saveStore(k, v);

  if (!loaded) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: bf, color: "#aaa" }}>Loading...</div>;

  // Admin password prompt
  if (adminPrompt && !adminAuth) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: bf, background: "#f5f4f2" }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: 36, maxWidth: 380, width: "90%", textAlign: "center", boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        ...
        <h2 style={{ fontFamily: hf, fontSize: 24, marginBottom: 6 }}>Admin Access</h2>
        ...
        <button onClick={() => { setAdminPrompt(false); window.location.hash = ""; }} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: bf, fontSize: 12, color: "#999" }}>Cancel</button>
      </div>
    </div>
  );

  if (mode === "admin" && adminAuth) return <AdminPanel products={products} setProducts={setProducts} hero={hero} setHero={setHero} pages={pages} setPages={setPages} site={site} setSite={setSite} cartCfg={cartCfg} setCartCfg={setCartCfg} uploadCfg={uploadCfg} setUploadCfg={setUploadCfg} design={design} setDesign={setDesign} orders={orders} setOrders={setOrders} save={save} onExit={() => { setMode("shop"); window.location.hash = ""; }} />;

  return (
    <div style={{ minHeight: "100vh", background: (design||DEF_DESIGN).bgColor, fontFamily: `'${(design||DEF_DESIGN).bodyFont}', sans-serif`, color: (design||DEF_DESIGN).textColor }}>
      <link href={`https://fonts.googleapis.com/css2?family=${encodeURIComponent((design||DEF_DESIGN).headingFont)}:ital,wght@0,300;0,400;0,500;0,600;0,700;1,400&family=${encodeURIComponent((design||DEF_DESIGN).bodyFont)}:ital,wght@0,300;0,400;0,500;0,600;0,700;1,400&display=swap`} rel="stylesheet" />
      {(design||DEF_DESIGN).customFontUrl && <link href={(design||DEF_DESIGN).customFontUrl} rel="stylesheet" />}
      ...
      {site.announcement?.enabled && site.announcement?.text && (
        <div style={{ background: site.announcement.bgColor || "#1a1a1a", color: site.announcement.textColor || "#fff", textAlign: "center", padding: "7px 12px", fontSize: site.announcement.fontSize || 12, fontWeight: 500, fontFamily: bf }}>
          {site.announcement.text}
        </div>
      )}

      {/* NAV */}
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: `${ds.navPadding}px ${ds.pagePaddingX}px`, borderBottom: `1px solid ${ds.borderColor}`, background: ds.navBg, backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 100, position: "relative" }}>
        <div onClick={() => { setStep(0); reset(); setSelProd(null); setCurPage(null); setMobileNav(false); }} style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 2 }}>
          {site.logoImage ? (
            <img src={site.logoImage} alt={site.logoText} style={{ height: site.logoHeight || 32, display: "block" }} />
          ) : (<>
            <span style={{ fontFamily: hf, fontSize: 24, fontStyle: "italic", color: ac }}>{site.logoText.slice(0, 2)}</span>
            <span style={{ fontFamily: hf, fontSize: 24 }}>{site.logoText.slice(2)}</span>
          </>)}
        </div>
      ...
      {site.testMode && (
        <div style={{ background: "#e8a020", color: "#fff", textAlign: "center", padding: "5px 12px", fontSize: 11, fontWeight: 600, fontFamily: bf, letterSpacing: 0.5 }}>⚠ TEST MODE — Orders will not be charged</div>
      )}

      {/* Photo Tray — persistent across all pages */}
      {uploadedPool.length > 0 && (
        <div style={{ background: "#f5f4f2", borderBottom: "1px solid rgba(0,0,0,0.06)", padding: "10px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, maxWidth: 980, margin: "0 auto" }}>
            {/* Add more button */}
            <button onClick={() => poolFileRef.current?.click()} style={{ width: 52, height: 52, borderRadius: 10, border: "2px dashed rgba(0,0,0,0.12)", background: "transparent", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2, flexShrink: 0 }}>
              <span style={{ fontSize: 18, color: "#ccc", lineHeight: 1 }}>+</span>
              <span style={{ fontSize: 8, color: "#aaa", fontFamily: bf, fontWeight: 600 }}>Add</span>
            </button>
            <input ref={poolFileRef} type="file" accept="image/*" multiple style={{ display: "none" }} onChange={e => {
              if (e.target.files.length > 1) handleFiles(e.target.files);
              else handleFile(e.target.files[0]);
              e.target.value = "";
            }} />
            {/* Photo thumbnails */}
            <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 2, flex: 1 }}>
              {uploadedPool.map((photo, i) => (
                <div key={i} style={{ position: "relative", flexShrink: 0 }}>
                  <div style={{ width: 52, height: 52, borderRadius: 10, overflow: "hidden", border: "2px solid rgba(0,0,0,0.06)", background: `url(${photo.url}) center/cover`, cursor: "pointer" }}
                    onClick={() => {
                      // Assign this photo to active frame of current product
                      if (selProd) {
                        setPhotosByProduct(prev => {
                          const current = prev[selProd.id] || [];
                          const n = [...current];
                          n[activeFrame] = { url: photo.url, name: photo.name, posX: 50, posY: 50, zoom: 100, orientation: photo.orientation };
                          return { ...prev, [selProd.id]: n };
                        });
                        setPhotoOrientation(photo.orientation || "landscape");
                        if (step !== 2) setStep(2);
                      }
                    }} />
                  <button onClick={e => { e.stopPropagation(); removeFromPool(i); }} style={{ position: "absolute", top: -4, right: -4, width: 18, height: 18, borderRadius: "50%", background: "rgba(0,0,0,0.5)", color: "#fff", border: "2px solid #f5f4f2", cursor: "pointer", fontSize: 10, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ opacity: fadeIn ? 1 : 0, transform: fadeIn ? "translateY(0)" : "translateY(8px)", transition: "all 0.5s ease" }}>
        ...
        {/* HOME */}
        {step === 0 && (<div>
          <div style={{ padding: `${hero.paddingTop || 72}px 24px ${hero.paddingBottom || 56}px`, textAlign: "center", position: "relative", overflow: "hidden", background: hero.bgImage ? `url(${hero.bgImage}) center/cover` : (hero.bgColor || "linear-gradient(180deg,#faf9f7,#f0ede8)") }}>
            {hero.bgImage && <div style={{ position: "absolute", inset: 0, background: `rgba(0,0,0,${(hero.overlayOpacity ?? 30) / 100})` }} />}
            <div style={{ position: "relative", zIndex: 1 }}>
              <p style={{ fontSize: hero.badgeSize || 11, letterSpacing: hero.badgeSpacing || 3, textTransform: "uppercase", color: hero.badgeColor || (hero.bgImage ? "rgba(255,255,255,0.7)" : "#aaa"), marginBottom: 16, fontWeight: 500 }}>{hero.badge}</p>
              <h1 style={{ fontFamily: (hero.headlineFont === "sans") ? F : S, fontSize: `clamp(28px,6vw,${hero.headlineSize || 56}px)`, fontWeight: hero.headlineWeight || 400, lineHeight: 1.1, margin: `0 auto 16px`, maxWidth: hero.maxWidth || 520, color: hero.headlineColor || (hero.bgImage ? "#fff" : "#1a1a1a") }}>
                {hero.headline}<br /><span style={{ fontStyle: hero.line2Italic !== false ? "italic" : "normal", color: hero.line2Color || undefined }}>{hero.headlineLine2}</span>
              </h1>
              <p style={{ fontSize: hero.subtitleSize || 15, color: hero.subtitleColor || (hero.bgImage ? "rgba(255,255,255,0.85)" : "#777"), maxWidth: hero.maxWidth ? hero.maxWidth - 100 : 400, margin: "0 auto 28px", lineHeight: 1.6 }}>{hero.subtitle}</p>
              <button onClick={() => setStep(1)} style={{ background: hero.ctaBg || ac, color: hero.ctaColor || "#fff", border: "none", padding: `${hero.ctaPaddingY || 14}px ${hero.ctaPaddingX || 40}px`, borderRadius: hero.ctaRadius ?? 50, fontSize: hero.ctaSize || 14, fontFamily: bf, fontWeight: 600, cursor: "pointer", boxShadow: "0 4px 16px rgba(0,0,0,0.15)" }}>{hero.ctaText}</button>
              {hero.showGuarantee !== false && <p style={{ fontSize: 11, color: hero.bgImage ? "rgba(255,255,255,0.5)" : "#bbb", marginTop: 12 }}>Free shipping on orders ${site.freeShipMin}+ · {site.guaranteeText}</p>}
            </div>
          </div>
          <div style={{ maxWidth: 900, margin: "0 auto", padding: "44px 24px 72px" }}>
            <h2 style={{ fontFamily: hf, fontSize: 28, textAlign: "center", marginBottom: 24 }}>Our Products</h2>
            <div style={{ display: "flex", gap: 6, justifyContent: "center", marginBottom: 24, flexWrap: "wrap" }}>
              {CATS.map(c => <button key={c} onClick={() => setShopCat(c)} style={{ padding: "7px 18px", borderRadius: 20, border: "none", cursor: "pointer", fontFamily: bf, fontSize: 12, fontWeight: 500, background: shopCat === c ? ac : "rgba(0,0,0,0.04)", color: shopCat === c ? "#fff" : "#888" }}>{c}</button>)}
            </div>
            <div className="grid-products" style={{ gridTemplateColumns: `repeat(auto-fill, minmax(${Math.round(900 / (ds.cardColumns || 3) - (ds.cardGap || 16))}px, 1fr))`, gap: ds.cardGap || 16 }}>
              {filtP.map(p => (<div key={p.id} onClick={() => { switchProduct(p); setStep(2); }} style={{ borderRadius: ds.cardRadius, overflow: "hidden", cursor: "pointer", background: ds.cardBg, border: ds.cardBorder !== false ? `1px solid ${ds.borderColor}` : "none", boxShadow: ds.cardShadow ? "0 4px 16px rgba(0,0,0,0.08)" : "none", transition: "all 0.3s" }}
                onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = "0 6px 20px rgba(0,0,0,0.08)"; }} onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = ds.cardShadow ? "0 4px 16px rgba(0,0,0,0.08)" : "none"; }}>
                <div style={{ height: ds.cardImgHeight, background: p.images?.[0] ? `url(${p.images[0]}) center/cover` : "linear-gradient(135deg,#ece9e4,#ddd8d0)", display: "flex", alignItems: "center", justifyContent: "center" }}>{(!p.images||!p.images.length) && <span style={{ fontSize: 32, opacity: 0.15 }}>📷</span>}</div>
                <div style={{ padding: `${ds.cardPadding}px` }}><div style={{ fontWeight: 600, fontSize: ds.cardNameSize, marginBottom: 3 }}>{p.name}</div><div style={{ fontSize: ds.cardDescSize, color: ds.mutedColor, marginBottom: 6, lineHeight: 1.4, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{p.desc}</div><div style={{ fontFamily: hf, fontSize: ds.cardPriceSize, color: ds.cardPriceColor || ds.textColor, fontWeight: ds.cardPriceWeight || 400 }}>From ${Math.min(...p.sizes.map(s => s.price))}</div></div>
              </div>))}
            </div>
          </div>
        </div>)}
          ...
          <div style={{ position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ maxWidth: 560, margin: "0 auto", padding: "44px 24px", position: "relative", zIndex: 1, display: "flex", flexDirection: uc.showProductPreview && uc.previewPosition === "left" ? "row" : uc.showProductPreview && uc.previewPosition === "right" ? "row-reverse" : "column", gap: 24, alignItems: "center" }}>
              ...
              <div style={{ flex: 1, textAlign: "center", background: uc.bgColor && !hasBg ? uc.bgColor : undefined }}>
                {selProd && <div style={{ marginBottom: 16 }}><p style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: hasBg ? "rgba(255,255,255,0.6)" : "#aaa", marginBottom: 3 }}>Creating on</p><h3 style={{ fontFamily: hf, fontSize: 24, color: hasBg ? "#fff" : "#1a1a1a" }}>{selProd.name}</h3></div>}
                <h2 style={{ fontFamily: hf, fontSize: 30, marginBottom: 6, color: hasBg ? "#fff" : "#1a1a1a" }}>{uc.headline}</h2>
        ...
        {step === 2 && (<div style={{ maxWidth: 980, margin: "0 auto", padding: "16px 24px 56px" }}>
          {selProd && !imageUrl && (() => {
            ...
            // Count frames for this product to show in upload hint
            const sc2 = (() => { const sz2 = selProd.sizes[0]; return sz2?.sceneLandscape || sz2?.scene || sz2?.scenePortrait || null; })();
            const frameCount = mkFrames(sc2).length;
            return (
            <div className="grid-customize">
              ...
              <div>
                {selProd.images?.[0] ? (
                  ...
                  <div style={{ borderRadius: 16, height: 240, background: "linear-gradient(135deg,#f0ebe4,#e0d8ce)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 48, opacity: 0.15 }}>📷</div>
                )}

                {/* Upload CTA — right under product image, compact with solid bg */}
                <div onDragOver={e => { e.preventDefault(); setDragO(true); }} onDragLeave={() => setDragO(false)} onDrop={handleDrop}
                  onClick={() => fileRef.current?.click()}
                  style={{ marginTop: 10, border: "none", borderRadius: 12, padding: "16px 18px", textAlign: "center", cursor: "pointer", background: dragO ? "#333" : ac, transition: "all 0.2s", display: "flex", alignItems: "center", gap: 12, justifyContent: "center", boxShadow: "0 4px 14px rgba(0,0,0,0.12)" }}>
                  <div style={{ fontSize: 20, color: "#fff", opacity: 0.7 }}>⬆</div>
                  <div style={{ textAlign: "left" }}>
                    <div style={{ fontWeight: 600, fontSize: 14, color: "#fff" }}>Upload {frameCount > 1 ? `${frameCount} photos` : "your photo"}</div>
                    <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 11 }}>JPG, PNG, or WEBP{frameCount > 1 ? " · Select multiple" : ""}</div>
                  </div>
              ...
              {/* Right: Product info */}
              <div>
                <h2 style={{ fontFamily: hf, fontSize: ds.pdpNameSize, marginBottom: 6 }}>{selProd.name}</h2>
                <div style={{ fontFamily: (ds.pdpPriceFont === "body") ? bf : hf, fontSize: ds.pdpPriceSize || 22, color: ds.pdpPriceColor || ac, fontWeight: ds.pdpPriceWeight || 400, marginBottom: 12 }}>From ${Math.min(...selProd.sizes.map(s => s.price))}</div>
                <p style={{ color: "#666", fontSize: 14, lineHeight: 1.7, marginBottom: 20, whiteSpace: "pre-wrap" }}>{selProd.fullDesc || selProd.desc}</p>
                {/* Attributes */}
                {selProd.attributes?.length > 0 && (
                  <div style={{ marginBottom: 20, display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {selProd.attributes.filter(a => a.label && a.value).map((attr, i) => (
                      <div key={i} style={{ padding: "6px 12px", borderRadius: 8, background: "#f5f4f2", border: "1px solid rgba(0,0,0,0.04)" }}>
                        <span style={{ fontSize: 11, color: "#999", fontWeight: 600 }}>{attr.label}: </span>
                        <span style={{ fontSize: 12, color: "#555" }}>{attr.value}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {selProd.sizes.map((s, i) => <span key={i} style={{ padding: "6px 12px", borderRadius: 7, background: "#f0ede8", fontSize: 12, color: "#555", fontFamily: bf }}>{s.label} — ${s.price}</span>)}
                  </div>
                </div>
                {/* More products */}
                <div>
                  ...
                  <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                    {active.filter(p => p.id !== selProd.id).map(p => <button key={p.id} onClick={() => switchProduct(p)} style={{ padding: "6px 10px", borderRadius: 7, cursor: "pointer", fontFamily: bf, fontSize: 11, fontWeight: 500, border: "1px solid rgba(0,0,0,0.08)", background: "#fff" }}>{p.name}</button>)}
                  </div>
                </div>
              </div>
            </div>);
          ...
          {selProd && imageUrl && (() => {
            ...
            <div className="grid-customize">
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, padding: "6px 10px", borderRadius: 8, background: "#f0ede8" }}>
                      ...
                      }} style={{ background: "none", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 6, padding: "2px 8px", cursor: "pointer", fontFamily: bf, fontSize: 10, color: "#888" }}>Reset</button>}
                    </div>
                  );
                })()}
              ...
              <div>
                <h3 style={{ fontFamily: hf, fontSize: ds.pdpNameSize || 24, marginBottom: 2 }}>{selProd.name}</h3>
                ...
                {selProd.attributes?.length > 0 && (
                  <div style={{ marginBottom: 14, display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {selProd.attributes.filter(a => a.label && a.value).map((attr, i) => (
                      <span key={i} style={{ padding: "4px 10px", borderRadius: 6, background: "#f5f4f2", fontSize: 11, color: "#666" }}>
                        <span style={{ fontWeight: 600, color: "#999" }}>{attr.label}:</span> {attr.value}
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                    {active.map(p => <button key={p.id} onClick={() => switchProduct(p)} style={{ padding: "6px 10px", borderRadius: 7, cursor: "pointer", fontFamily: bf, fontSize: 11, fontWeight: 500, border: selProd.id === p.id ? `2px solid ${ac}` : "1px solid rgba(0,0,0,0.08)", background: selProd.id === p.id ? "#faf9f7" : "#fff" }}>{p.name}</button>)}
                  </div>
                </div>
                ...
                <div style={{ background: "#fff", borderRadius: 12, padding: 16, border: "1px solid rgba(0,0,0,0.06)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
                    ...
                    <span style={{ fontFamily: (ds.pdpPriceFont === "body") ? bf : hf, fontSize: ds.pdpPriceSize || 24, color: ds.pdpPriceColor || ds.textColor, fontWeight: ds.pdpPriceWeight || 400 }}>${price}</span>
                  </div>
                  <button onClick={addToCart} style={{ width: "100%", background: ac, color: "#fff", border: "none", padding: 13, borderRadius: 10, fontSize: 14, fontFamily: bf, fontWeight: 600, cursor: "pointer" }}>Add to Cart</button>
                </div>
              </div>
            </div>);
          })()}
        ...
        {step === 3 && (<div style={{ maxWidth: 500, margin: "0 auto", padding: "44px 24px" }}>
          <h2 style={{ fontFamily: hf, fontSize: 28, marginBottom: 24 }}>Your cart</h2>
          {cart.length === 0 ? (<div style={{ textAlign: "center", padding: "52px 0" }}><p style={{ color: "#aaa", marginBottom: 18 }}>Your cart is empty</p><button onClick={() => setStep(1)} style={{ background: ac, color: "#fff", border: "none", padding: "13px 36px", borderRadius: 50, fontFamily: bf, fontWeight: 600, cursor: "pointer", fontSize: 13 }}>Start Creating</button></div>) : (<>
            {cart.map((item, i) => (<div key={item.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ fontFamily: hf, fontSize: 15 }}>${item.price}</div>
            ...
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: `2px solid ${ac}` }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 17, fontWeight: 600, marginBottom: 20 }}><span>Total</span><span style={{ fontFamily: hf, fontSize: 24 }}>${(cartTot + tax + shipCost).toFixed(2)}</span></div>
              <button onClick={() => setStep(6)} style={{ width: "100%", background: ac, color: "#fff", border: "none", padding: 14, borderRadius: 10, fontSize: 14, fontFamily: bf, fontWeight: 600, cursor: "pointer" }}>Proceed to Checkout</button>
              <button onClick={() => setStep(1)} style={{ width: "100%", background: "none", border: "none", padding: 9, fontSize: 12, color: "#aaa", cursor: "pointer", fontFamily: bf, marginTop: 3 }}>+ Add another</button>
            </div>
          </>)}
        </div>)}
        ...
        {step === 6 && (<div style={{ maxWidth: 720, margin: "0 auto", padding: "36px 24px 60px" }}>
          <button onClick={() => setStep(3)} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: bf, fontSize: 13, color: "#888", marginBottom: 16 }}>← Back to cart</button>
          <h2 style={{ fontFamily: hf, fontSize: 28, marginBottom: 24 }}>Checkout</h2>
          <div className="grid-checkout">
            ...
            <div>
              ...
              <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 16 }}>
                <h3 style={{ fontFamily: hf, fontSize: 20, marginBottom: 14 }}>Contact</h3>
              ...
              {/* Shipping */}
              <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 16 }}>
                <h3 style={{ fontFamily: hf, fontSize: 20, marginBottom: 14 }}>Shipping Address</h3>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
                  <div>
              ...
              {/* Payment */}
              <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)" }}>
                <h3 style={{ fontFamily: hf, fontSize: 20, marginBottom: 14 }}>Payment</h3>
            ...
            <div style={{ position: "sticky", top: 70 }}>
              <div style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid rgba(0,0,0,0.06)" }}>
                <h3 style={{ fontFamily: hf, fontSize: 18, marginBottom: 14 }}>Order Summary</h3>
                {cart.map((item, i) => (
                  <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < cart.length - 1 ? "1px solid rgba(0,0,0,0.04)" : "none" }}>
                    ...
                    <div style={{ fontFamily: hf, fontSize: 14 }}>${item.price}</div>
                  </div>
                ))}
                <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid rgba(0,0,0,0.06)" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 16, fontWeight: 700, paddingTop: 8, borderTop: "1px solid rgba(0,0,0,0.06)" }}><span>Total</span><span style={{ fontFamily: hf, fontSize: 22 }}>${(cartTot + tax + shipCost).toFixed(2)}</span></div>
                </div>
                {/* Test order button */}
                {site.testMode && (
                  <button onClick={async () => {
                    setGenerating(true);
                    try {
                      const orderId = "TEST-" + uid().toUpperCase();
                      const orderItems = [];
                      const allPdfs = [];
                      for (const item of cart) {
                        ...
                        if (isMulti) {
                          const results = await generateMultiPrintPDFs(item);
                          for (const r of results) {
                            const pdfUrl = URL.createObjectURL(r.blob);
                            allPdfs.push({ name: `${item.product.name}-${item.size.replace(/["""]/g, "")}-photo${r.photoIndex+1}-${orderId}.pdf`, url: pdfUrl, size: item.size, product: item.product.name, dims: r.dims, dpi: r.dpi, px: { w: r.pxW, h: r.pxH } });
                          }
                          orderItems.push({ ...item, pdfCount: results.length, pdfDims: results[0]?.dims, pdfDpi: results[0]?.dpi, pdfPx: results[0] ? { w: results[0].pxW, h: results[0].pxH } : null, product: { id: item.product.id, name: item.product.name, category: item.product.category, printMode: item.product.printMode } });
                        } else {
                          const { blob, dims, dpi, pxW, pxH } = await generatePrintPDF(item);
                          const pdfUrl = URL.createObjectURL(blob);
                          allPdfs.push({ name: `${item.product.name}-${item.size.replace(/["""]/g, "")}-${orderId}.pdf`, url: pdfUrl, size: item.size, product: item.product.name, dims, dpi, px: { w: pxW, h: pxH } });
                          orderItems.push({ ...item, pdfDims: dims, pdfDpi: dpi, pdfPx: { w: pxW, h: pxH }, product: { id: item.product.id, name: item.product.name, category: item.product.category, printMode: item.product.printMode } });
                        }
                      }
                      const order = {
                        id: orderId, isTest: true,
                        customer: { email: checkout.email || "test@test.com", firstName: checkout.firstName || "Test", lastName: checkout.lastName || "Order", phone: "", address: checkout.address || "123 Test St", city: checkout.city || "Test City", state: checkout.state || "FL", zip: checkout.zip || "33414" },
                        payment: { cardLast4: "0000", cardName: "TEST ORDER" },
                        items: orderItems.map(it => ({ imageName: it.imageName, imageThumb: it.imageUrl ? it.imageUrl.substring(0, 100) + "..." : null, size: it.size, filter: "none", price: it.price, pdfDims: it.pdfDims, pdfDpi: it.pdfDpi, pdfPx: it.pdfPx, pdfCount: it.pdfCount, photoCount: it.photos?.filter(p => p?.url).length || 1, product: { id: it.product.id, name: it.product.name, category: it.product.category } })),
                      ...
                      window._lastOrderPdfs = allPdfs;
                      window._lastOrderId = orderId;
                      setStep(4); setCart([]);
                    } catch (err) { console.error("Test order failed:", err); alert("Error: " + err.message); }
                    setGenerating(false);
                  }} disabled={generating} style={{ width: "100%", marginTop: 16, background: generating ? "#999" : "#e8a020", color: "#fff", border: "none", padding: 15, borderRadius: 10, fontSize: 15, fontFamily: bf, fontWeight: 700, cursor: generating ? "wait" : "pointer" }}>
                    {generating ? "Processing..." : `⚠ Place Test Order — $${(cartTot + tax + shipCost).toFixed(2)}`}
                  </button>
                )}
                <button onClick={async () => {
                  // Validate
                  ...
                  try {
                    ...
                    const order = {
                      ...
                      items: orderItems.map(it => ({ imageName: it.imageName, size: it.size, price: it.price, pdfDims: it.pdfDims, pdfDpi: it.pdfDpi, pdfPx: it.pdfPx, pdfCount: it.pdfCount, photoCount: it.photos?.filter(p => p?.url).length || 1, product: { id: it.product.id, name: it.product.name, category: it.product.category, printMode: it.product.printMode } })),
                      total: (cartTot + tax + shipCost).toFixed(2), subtotal: cartTot.toFixed(2), tax: tax.toFixed(2), shipping: shipCost.toFixed(2),
                      status: "pending", createdAt: new Date().toISOString(), itemCount: cart.length,
                    };
                    const newOrders = [...orders, order];
                  ...
                  setGenerating(false);
                }} disabled={generating} style={{ width: "100%", marginTop: 16, background: generating ? "#999" : ac, color: "#fff", border: "none", padding: 15, borderRadius: 10, fontSize: 15, fontFamily: bf, fontWeight: 700, cursor: generating ? "wait" : "pointer", letterSpacing: 0.3 }}>
        ...
        {step === 4 && (<div style={{ maxWidth: 500, margin: "0 auto", padding: "60px 24px", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: hf, fontSize: 32, marginBottom: 8 }}>Order placed!</h2>
            ...
            <div style={{ background: "#fff", borderRadius: 14, padding: 20, textAlign: "left", border: "1px solid rgba(0,0,0,0.06)", marginBottom: 24 }}>
              ...
              {window._lastOrderPdfs.map((pdf, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < window._lastOrderPdfs.length - 1 ? "1px solid rgba(0,0,0,0.05)" : "none" }}>
                  ...
                  <a href={pdf.url} download={pdf.name} style={{ padding: "6px 14px", borderRadius: 8, background: ac, color: "#fff", textDecoration: "none", fontSize: 12, fontFamily: bf, fontWeight: 600, flexShrink: 0 }}>Download</a>
                </div>
              ))}
            </div>
          )}
          ...
          <button onClick={() => { setStep(0); reset(); setSelProd(null); window._lastOrderPdfs = null; window._lastOrderId = null; }} style={{ background: ac, color: "#fff", border: "none", padding: "13px 36px", borderRadius: 50, fontFamily: bf, fontWeight: 600, cursor: "pointer", fontSize: 13 }}>Back to Home</button>
        </div>)}

        {/* CUSTOM PAGE */}
        {step === 5 && curPage && <PageRenderer page={curPage} products={products} onProductClick={p => { switchProduct(p); setStep(2); }} />}

      </div>

      <footer style={{ borderTop: `1px solid ${ds.borderColor}`, padding: `${ds.pagePaddingX}px`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, fontSize: ds.footerFontSize, color: ds.footerTextColor, background: ds.footerBg }}>
...
const inp = { width: "100%", padding: "9px 12px", borderRadius: 9, border: "1px solid rgba(0,0,0,0.12)", fontFamily: bf, fontSize: 13, outline: "none", boxSizing: "border-box", background: "#fff" };
...
const navB = { background: "none", border: "none", cursor: "pointer", fontFamily: bf, fontSize: 13, color: "#555", fontWeight: 500 };
const btnP = { padding: "10px 22px", borderRadius: 9, border: "none", background: "#1a1a1a", color: "#fff", cursor: "pointer", fontFamily: bf, fontSize: 13, fontWeight: 600 };
const btnO = { padding: "10px 20px", borderRadius: 9, border: "1px solid rgba(0,0,0,0.12)", background: "#fff", cursor: "pointer", fontFamily: bf, fontSize: 13 };
